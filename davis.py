        ...
        with open(os.path.join(_imset_f), "r") as lines:
            for line in lines:
                ...
                _mask = np.array(Image.open(os.path.join(
                    self.mask_dir, '00000.png')).convert("P"))
    ...
    def __getitem__(self, index):
        ...
        for f in f_list:
            ...
            img_file = os.path.join(
                self.image_dir, '{:05d}.jpg'.format(f))
            image_ = cv2.resize(
            ...
            try:
                mask_file = os.path.join(
                    self.mask_dir, '{:05d}.png'.format(f))
            except:
